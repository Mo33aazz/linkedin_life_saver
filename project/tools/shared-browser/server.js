...
const logger = Object.keys(LOG_LEVELS).reduce((acc, level) => {
  acc[level] = (message, context = {}) => {
    ...
    console.log(
      `${new Date().toLocaleTimeString()} ${color}${levelStr}${COLORS.reset} - ${message}`
    );
...
function parseExtensionDirs() {
  ...
  if (idxExts !== -1 && args[idxExts + 1]) {
    for (const p of String(args[idxExts + 1]).split(','))
  ...
  const envExt =
    process.env.SHARED_EXTENSIONS_DIRS || process.env.EXTENSION_DIRS || '';
...
const DEFAULT_TIMEOUT_MS = Number(
  process.env.SHARED_BROWSER_DEFAULT_TIMEOUT_MS || 45000
);
...
function enqueue(pageId, task) {
  ...
  const queue =
    queueKey === 'global'
      ? globalQueue
      : queues.get(queueKey) || { p: Promise.resolve() };
  ...
  const next = queue.p
    .then(() => {
      ...
      if (pageId && !pages.has(String(pageId))) {
        throw new Error(
          `Page ${pageId} was closed before the action could run.`
        );
      }
      return task();
    })
    .catch((e) => {
      ...
      logger.error(
        `${scope.charAt(0).toUpperCase() + scope.slice(1)} queue error: ${String(e)}`,
        { type: 'queue', scope, pageId }
      );
...
async function startBrowser() {
  ...
  if (extensionDirs.length) {
    ...
    logger.info(`Loading extensions: ${extensionDirs.join(' | ')}`, {
      type: 'startup',
  ...
  const preferredChannels = (
    process.env.SHARED_BROWSER_CHANNELS || 'chrome,chrome-beta,msedge'
  )
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  if (chromePath) {
    try {
      ...
      logger.info(`Using Chrome at CHROME_PATH=${chromePath}`, {
        type: 'startup',
      });
    } catch (e) {
      logger.error(`Failed to launch with CHROME_PATH: ${String(e)}`, {
        type: 'startup',
      });
    }
  }
  ...
  for (const channel of launched ? [] : preferredChannels) {
    try {
      ...
      logger.warn(`Channel ${channel} failed: ${String(e)}`, {
        type: 'startup',
      });
    }
  }
  ...
  if (!launched) {
    logger.info('Launching with bundled Chromium fallback', {
      type: 'startup',
    });
    launched = await chromium.launchPersistentContext(
      USER_DATA_DIR,
      commonLaunchOpts
    );
  }
...
function attachPage(page) {
  ...
  try {
    ...
    logger.error(
      `Failed to set default timeouts on page ${pageId}: ${String(e)}`,
      { pageId, error: String(e) }
    );
  }
  ...
  page.on('console', (msg) => {
    ...
    const suppressInvalid = (
      process.env.SHARED_BROWSER_SUPPRESS_INVALID_EXT_ERRORS || '1'
    ).toLowerCase();
    ...
    if (shouldSuppress && text === 'Failed to load resource: net::ERR_FAILED') {
      logger.debug(`[Console/${severity}] ${text} (suppressed)`, {
        type: 'page:console:suppressed',
        pageId,
        text,
        severity,
      });
...
async function handleAction(action, payload) {
  switch (action) {
    ...
    case 'waitForSelector': {
      ...
      if (!page) throw new Error(`Unknown pageId ${pageId}`);
      return enqueue(String(pageId), async () => {
        logger.info(`Action: waitForSelector '${selector}'`, {
          type: 'action:waitForSelector',
          pageId,
          selector,
          state,
        });
    ...
    case 'type': {
      ...
      if (!page) throw new Error(`Unknown pageId ${pageId}`);
      return enqueue(String(pageId), async () => {
        logger.info(`Action: type '${text}' into '${selector}'`, {
          type: 'action:type',
          pageId,
          selector,
        });
    ...
    case 'fill': {
      ...
      if (!page) throw new Error(`Unknown pageId ${pageId}`);
      return enqueue(String(pageId), async () => {
        logger.info(`Action: fill '${selector}'`, {
          type: 'action:fill',
          pageId,
          selector,
        });
    ...
    case 'evaluate': {
      ...
      if (!page) throw new Error(`Unknown pageId ${pageId}`);
      return enqueue(String(pageId), async () => {
        logger.info(`Action: evaluate expression`, {
    ...
    case 'closePage': {
      ...
      if (!page) throw new Error(`Unknown pageId ${pageId}`);
      return enqueue(String(pageId), async () => {
        logger.info(`Action: closePage ${pageId}`, {
          type: 'action:closePage',
          pageId,
        });
        await page.close();
        return { ok: true };
    ...
    case 'evaluateOnServiceWorker': {
      ...
      return enqueue(null, async () => {
        logger.info(`Action: evaluateOnServiceWorker`, {
          type: 'action:evaluateOnServiceWorker',
        });
        const pickExtensionSW = () => {
          ...
          const anyExt = workers.find((w) => {
            try {
              return w.url().startsWith('chrome-extension://');
            } catch {
              return false;
            }
          });
        ...
        logger.debug(`Evaluating on SW: ${url}`, {
          type: 'action:evaluateOnServiceWorker:target',
          url,
        });
...
async function main() {
  ...
  async function listenWithRetry(startPort) {
    ...
    throw new Error(
      `Unable to bind on range starting ${startPort} after ${maxAttempts} attempts`
    );
  }
  ...
  async function gracefulExit(signal) {
    logger.info(`${signal} received, closing browser and server.`, {
      type: 'shutdown',
    });
    try {
      if (browserContext) {
