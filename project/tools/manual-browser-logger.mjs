...
const userDataDir =
  process.env.USER_DATA_DIR || path.join(logDir, `chromium-profile-${ts}`);
...
async function bindPage(page) {
  ...
  await page.addInitScript(() => {
    const describe = (el) => {
      ...
      const text = (el.innerText || el.value || '')
        ...
        .trim()
        .slice(0, 120);
      const attrs = ['id', 'name', 'type', 'role', 'placeholder'].reduce(
        (a, k) => {
          try {
    ...
    window.addEventListener(
      ...
      (e) => {
        ...
        const isPw =
          ...
          (t.type === 'password' ||
            (t.getAttribute &&
              /passw/i.test(String(t.getAttribute('name') || ''))));
        const val =
          !isPw &&
          t &&
          (t.value !== undefined ? String(t.value).slice(0, 200) : undefined);
        send({
          ev: 'input',
          target: describe(t),
          valuePreview: val,
    ...
    window.addEventListener(
      ...
      (e) => {
        ...
        const isPw =
          ...
          (ae.type === 'password' ||
            (ae.getAttribute &&
              /passw/i.test(String(ae.getAttribute('name') || ''))));
        send({
          ev: 'key',
          key: isPw ? undefined : e.key,
          code: e.code,
          ctrl: e.ctrlKey,
          alt: e.altKey,
          meta: e.metaKey,
    ...
    window.addEventListener(
      'submit',
      (e) => send({ ev: 'submit', target: describe(e.target) }),
      true
    );
    window.addEventListener(
      'selectionchange',
    ...
    window.addEventListener(
      'hashchange',
      () => send({ ev: 'hashchange', url: location.href }),
      true
    );
    document.addEventListener(
      'visibilitychange',
      () => send({ ev: 'visibility', state: document.visibilityState }),
      true
    );
  ...
  page.on('framenavigated', (frame) => {
    if (frame === page.mainFrame()) append({ type: 'nav', url: frame.url() });
  });
  page.on('dialog', (d) =>
    append({ type: 'dialog', kind: d.type(), message: d.message() })
  );
  page.on('console', (msg) =>
    append({ type: 'console', level: msg.type(), text: msg.text() })
  );
  page.on('filechooser', (fc) =>
    append({ type: 'filechooser', isMultiple: fc.isMultiple() })
  );
}
...
(async () => {
  ...
  await context.addInitScript(() => {
    try {
      ...
      if (navigator.permissions && navigator.permissions.query) {
        const original = navigator.permissions.query.bind(
  ...
  context.on('response', async (res) => {
    ...
    try {
      ...
      size = Number(headers['content-length']) || undefined;
    } catch {}
    append({ type: 'response', status: res.status(), url: res.url(), size });
  });
  context.on('requestfailed', (req) =>
    append({ type: 'request_failed', url: req.url(), failure: req.failure() })
  );
  ...
  context.on('websocket', (ws) => {
    ...
    ws.on('framesent', (e) =>
      append({ type: 'ws_framesent', length: e.payloadData?.length })
    );
    ws.on('framereceived', (e) =>
      append({ type: 'ws_framereceived', length: e.payloadData?.length })
    );
  ...
  process.on('SIGINT', async () => {
    append({ type: 'signal', sig: 'SIGINT' });
    await context.close();
  });
  process.on('SIGTERM', async () => {
    append({ type: 'signal', sig: 'SIGTERM' });
    await context.close();
  });
  ...
  let cmdPos = 0;
  const getActivePage = () =>
    context.pages()[context.pages().length - 1] || context.pages()[0];
    ...
    try {
      if (cmd.cmd === 'navigate' && cmd.url) {
        ...
        await p
          ...
          .first()
          .click({ timeout: cmd.timeout || 10000 });
        append({
          type: 'action_done',
          name: 'click_selector',
          selector: cmd.selector,
