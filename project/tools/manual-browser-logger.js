...
(async () => {
  ...
  const bindPage = async (page) => {
    ...
    await page.exposeBinding('__logEvent', (source, payload) => {
      log({
        type: 'dom_event',
        pageId: page._guid || page.guid || undefined,
        url: page.url(),
        ...payload,
      });
    });
    ...
    await page.addInitScript(() => {
      const describe = (el) => {
        ...
        const attrs = [
          ...
          if (el.getAttribute && el.getAttribute(k) != null)
            a[k] = el.getAttribute(k);
          return a;
        }, {});
        return {
          tag: el.tagName,
          ...
          selector: (() => {
            try {
              return window.getComputedStyle ? undefined : undefined;
            } catch {
              return undefined;
            }
          })(),
    ...
    page.on('dialog', (d) =>
      log({ type: 'dialog', kind: d.type(), message: d.message() })
    );
    page.on('console', (msg) =>
      log({ type: 'console', level: msg.type(), text: msg.text() })
    );
    page.on('filechooser', (fc) =>
      log({ type: 'filechooser', isMultiple: fc.isMultiple() })
    );
  };
  ...
  context.on('requestfailed', (req) =>
    log({ type: 'request_failed', url: req.url(), failure: req.failure() })
  );
  ...
  context.on('websocket', (ws) => {
    ...
    ws.on('framesent', (e) =>
      log({ type: 'ws_framesent', length: e.payloadData?.length })
    );
    ws.on('framereceived', (e) =>
      log({ type: 'ws_framereceived', length: e.payloadData?.length })
    );
  ...
  process.on('SIGINT', async () => {
    log({ type: 'signal', sig: 'SIGINT' });
    await context.close();
  });
  process.on('SIGTERM', async () => {
    log({ type: 'signal', sig: 'SIGTERM' });
    await context.close();
  });
