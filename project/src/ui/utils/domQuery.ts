...
export function getTreeRoot(node: Node): Document | ShadowRoot {
  ...
  if (root && (root as Document | ShadowRoot).nodeType === Node.DOCUMENT_FRAGMENT_NODE) {
...
export function getById<T extends HTMLElement = HTMLElement>(start: Element, id: string): T | null {
...
export function query<T extends Element = Element>(start: Element, selector: string): T | null {
...
export function queryAll<T extends Element = Element>(start: Element, selector: string): T[] {
